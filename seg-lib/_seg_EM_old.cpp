        ...
        for (long Multispec = 0; Multispec < this->nu; Multispec++) {
            ...
            if (OutliernessFlag) {
                if (BiasField != NULL) {
                    ...
                    for (int i = 0;
                         i < numel_masked; i++, ExpectationPTR++, OutliernessPTR++, BiasFieldPTR++, S2L_PTR++) {
                        segPrecisionTYPE current_value =
                                (*ExpectationPTR) * (*OutliernessPTR) * (InputImagePtr[(*S2L_PTR)] + (*BiasFieldPTR));
                    ...
                    for (int i = 0; i < numel_masked; i++, ExpectationPTR++, S2L_PTR++) {
                        segPrecisionTYPE current_value =
                                (*ExpectationPTR) * (*OutliernessPTR) * (InputImagePtr[(*S2L_PTR)]);
                        if (current_value == current_value) {
                            tempsum += current_value;
                            SumPriors += (*ExpectationPTR) * (*OutliernessPTR);
                        }
                ...
                if (BiasField != NULL) {
                    ...
                    for (int i = 0; i < numel_masked; i++, ExpectationPTR++, BiasFieldPTR++, S2L_PTR++) {
                        segPrecisionTYPE current_value =
                                (*ExpectationPTR) * (InputImagePtr[(*S2L_PTR)] + (*BiasFieldPTR));
                        if (current_value == current_value) {
                            tempsum += current_value;
            ...
            if (SumPriors == SumPriors && SumPriors > 0) {
                if (this->mapStatus && this->MAP_M != NULL) {
                    // The mean M
                    M[cl * (this->nu) + Multispec] = (tempsum / SumPriors / powf(V[cl * (this->nu) + Multispec], 2) +
                                                      this->MAP_M[cl * (this->nu) + Multispec] /
                                                      powf(this->MAP_V[cl * (this->nu) + Multispec], 2)) /
                                                     (1 / powf(V[cl * (this->nu) + Multispec], 2) +
                                                      1 / powf(this->MAP_V[cl * (this->nu) + Multispec], 2));
                } else {
                ...
                for (long Multispec2 = Multispec; Multispec2 < this->nu; Multispec2++) {
                    ...
                    if (BiasField != NULL) {
                        ...
                        if (OutliernessFlag) {
                            for (int i = 0; i <
                                            numel_masked; i++, ExpectationPTR++, BiasFieldPTR++, OutliernessPTR++, BiasField_PTR2++, S2L_PTR++) {

                                segPrecisionTYPE currentValue = (*ExpectationPTR) * (*OutliernessPTR) *
                                                                (InputImagePtr[(*S2L_PTR)] + (*BiasFieldPTR) - tmpM) *
                                                                (T1_PTR2[(*S2L_PTR)] + (*BiasField_PTR2) - tmpM2);
                            ...
                            for (int i = 0;
                                 i < numel_masked; i++, ExpectationPTR++, BiasFieldPTR++, BiasField_PTR2++, S2L_PTR++) {
                                segPrecisionTYPE currentValue =
                                        (*ExpectationPTR) * (InputImagePtr[(*S2L_PTR)] + (*BiasFieldPTR) - tmpM) *
                                        (T1_PTR2[(*S2L_PTR)] + (*BiasField_PTR2) - tmpM2);
                                if (currentValue == currentValue) {
                                    tempsum += currentValue;
                                }
                        ...
                        for (int i = 0; i < numel_masked; i++, ExpectationPTR++, S2L_PTR++) {
                            segPrecisionTYPE current_vaue = (*ExpectationPTR) * (InputImagePtr[(*S2L_PTR)] - tmpM) *
                                                            (T1_PTR2[(*S2L_PTR)] - tmpM2);
                    ...
                    if ((tempsum / SumPriors > 0) && SumPriors > 0 && (!isnan(tempsum / SumPriors))) {
                        ...
                        if (Multispec2 != Multispec) {
                            ...
                            V[cl * this->nu * this->nu + Multispec2 + Multispec * this->nu] = V[
                                    cl * this->nu * this->nu + Multispec + Multispec2 * this->nu];
        ...
        for (iy = 1; iy < maxiy - 1; iy++) {
            ...
            for (ix = 1; ix < maxix - 1; ix++) {
                ...
                if (curr_short_centreindex >= 0) {
                    ...
                    indexWest =
                            Long_2_Short_IndicesPtr[indexCentre - col_size] > -1 ? Long_2_Short_IndicesPtr[indexCentre -
                                                                                                           col_size]
                                                                                 : 0;
                    indexEast =
                            Long_2_Short_IndicesPtr[indexCentre + col_size] > -1 ? Long_2_Short_IndicesPtr[indexCentre +
                                                                                 ...
                                                                                 : 0;
                    indexNorth =
                            Long_2_Short_IndicesPtr[indexCentre - 1] > -1 ? Long_2_Short_IndicesPtr[indexCentre - 1]
                                                                          : 0;
                    indexSouth =
                            Long_2_Short_IndicesPtr[indexCentre + 1] > -1 ? Long_2_Short_IndicesPtr[indexCentre + 1]
                                                                          : 0;
                    ...
                    for (currclass = 0; currclass < numclass; currclass++) {
                        for (neighbourclass = 0; neighbourclass < numclass; neighbourclass++) {
                            Temp_MRF_Class_Expect[currclass] -=
                                    G[currclass + (numclass) * neighbourclass] * Gplane[neighbourclass];
                        }

                        if (this->MRFBeta == NULL) {
                            Temp_MRF_Class_Expect[currclass] = exp(Temp_MRF_Class_Expect[currclass]) *
                                                               this->ShortPrior[curr_short_centreindex +
                                                                                numelmasked_currclass_shift[currclass]];
                        } else {
                            Temp_MRF_Class_Expect[currclass] =
                                    exp(this->MRFBeta[curr_short_centreindex] * Temp_MRF_Class_Expect[currclass]) *
                                    this->ShortPrior[curr_short_centreindex + numelmasked_currclass_shift[currclass]];
                        }
                    ...
                    for (currclass = 0; currclass < numclass; currclass++) {
                        MRFpriorPtr[curr_short_centreindex + numelmasked_currclass_shift[currclass]] = (
                                Temp_MRF_Class_Expect[currclass] / Sum_Temp_MRF_Class_Expect);
                    }
                }
            }
        }
            ...
            for (int iy = 1; iy < maxiy - 1; iy++) {
                ...
                for (int ix = 1; ix < maxix - 1; ix++) {
                    ...
                    if (curr_short_centreindex >= 0) {
                        ...
                        indexWest = Long_2_Short_IndicesPtr[indexCentre - col_size] > -1 ? Long_2_Short_IndicesPtr[
                                indexCentre - col_size] : 0;
                        indexEast = Long_2_Short_IndicesPtr[indexCentre + col_size] > -1 ? Long_2_Short_IndicesPtr[
                                indexCentre + col_size] : 0;
                        indexNorth =
                                Long_2_Short_IndicesPtr[indexCentre - 1] > -1 ? Long_2_Short_IndicesPtr[indexCentre - 1]
                                                                              : 0;
                        indexSouth =
                                Long_2_Short_IndicesPtr[indexCentre + 1] > -1 ? Long_2_Short_IndicesPtr[indexCentre + 1]
                                                                              : 0;
                        indexBottom = Long_2_Short_IndicesPtr[indexCentre + plane_size] > -1 ? Long_2_Short_IndicesPtr[
                                indexCentre + plane_size] : 0;
                        indexTop = Long_2_Short_IndicesPtr[indexCentre - plane_size] > -1 ? Long_2_Short_IndicesPtr[
                                indexCentre - plane_size] : 0;
                        for (currclass = 0; currclass < numclass; currclass++) {
                            // Get the sum of the expectations for each class over all the neighbours
                            Gplane[currclass] = 0.0;
                        ...
                        for (currclass = 0; currclass < numclass; currclass++) {
                            for (int neighbourclass = 0; neighbourclass < numclass; neighbourclass++) {
                                Temp_MRF_Class_Expect[currclass] -=
                                        G[currclass + (numclass) * neighbourclass] * Gplane[neighbourclass] +
                                        H[currclass + (numclass) * neighbourclass] * Hplane[neighbourclass];
                            }
                            if (this->MRFBeta == NULL) {
                                Temp_MRF_Class_Expect[currclass] = exp(Temp_MRF_Class_Expect[currclass]) *
                                                                   this->ShortPrior[curr_short_centreindex +
...
void seg_EM_old::RunBiasField3D() {
    ...
    int PowerOrder[
            (int) ((((float) (maxAllowedBCPowerOrder) + 1.0f) * ((float) (maxAllowedBCPowerOrder) + 2.0f) / 2.0f *
                    ((float) (maxAllowedBCPowerOrder) + 3.0f) / 3.0f)) * 3] = {0};
    int ind = 0;
    ...
    for (long multispec = 0; multispec < this->nu; multispec++) {
        ...
        //
        segPrecisionTYPE AWA[
                (int) ((((float) (maxAllowedBCPowerOrder) + 1.0f) * ((float) (maxAllowedBCPowerOrder) + 2.0f) / 2.0f *
                        ((float) (maxAllowedBCPowerOrder) + 3.0f) / 3.0f)) *
                (int) ((((float) (maxAllowedBCPowerOrder) + 1.0f) * ((float) (maxAllowedBCPowerOrder) + 2.0f) / 2.0f *
                        ((float) (maxAllowedBCPowerOrder) + 3.0f) / 3.0f))] = {0.0f};
        segPrecisionTYPE WR[(int) ((((float) (maxAllowedBCPowerOrder) + 1.0f) *
                                    ((float) (maxAllowedBCPowerOrder) + 2.0f) / 2.0f *
                                    ((float) (maxAllowedBCPowerOrder) + 3.0f) / 3.0f))] = {0.0f};
        segPrecisionTYPE FinalCoefs[(int) ((((float) (maxAllowedBCPowerOrder) + 1.0f) *
                                            ((float) (maxAllowedBCPowerOrder) + 2.0f) / 2.0f *
                                            ((float) (maxAllowedBCPowerOrder) + 3.0f) / 3.0f))] = {0.0f};
        ...
        if (this->numelBias == 0) {
            ...
            if (verbose_level > 0) {
                cout << "Number of samples for BiasField = " << samplecount << ", with " << UsedBasisFunctions
                     << " basis functions\n";
                flush(cout);
            }
        ...
        try {
            ...
            std::cerr << "ERROR:Low memory, could not allocate Q (" << samplecount << " float elements): " << ba.what()
                      << std::endl;
            seg_exit();
        }
                ...
                for (int ix = 0; ix < maxix; ix += reduxfactor) {
                    ...
                    if (currshortindex >= 0) {
                        ...
                        if (Outlierness == NULL) {
                            ...
                            for (int j = 0; j < this->numberOfClasses; j++) {
                                W_tmp += Expec[currshortindex + this->numElementsMasked * j] *
                                         Outlierness[currshortindex + this->numElementsMasked * j] * invV[j];
                            }
        ...
        try {
            ...
            std::cerr << "ERROR:Low memory, could not allocate Basis (" << UsedBasisFunctions << " float elements): "
                      << ba.what() << std::endl;
            seg_exit();
        }
            ...
            for (int iy = 0; iy < maxiy; iy += reduxfactor) {
                for (int ix = 0; ix < maxix; ix += reduxfactor) {
                    ...
                    if (currshortindex >= 0) {
                        ...
                        currentW = W[Windex];
                        xpos = (((segPrecisionTYPE) ix - not_point_five_times_dims_x) *
                                inv_not_point_five_times_dims_x);
                        ypos = (((segPrecisionTYPE) iy - not_point_five_times_dims_y) *
                                inv_not_point_five_times_dims_y);
                        zpos = (((segPrecisionTYPE) iz - not_point_five_times_dims_z) *
                                inv_not_point_five_times_dims_z);
                        x_bias_index_shift = 0;
                        y_bias_index_shift = 1;
                        z_bias_index_shift = 2;
                        for (int j2 = 0; j2 <
                                         UsedBasisFunctions; j2++, x_bias_index_shift += 3, y_bias_index_shift += 3, z_bias_index_shift += 3, Basisptr1++) {
                            // Because Powerorder is always int, use a special power function (faster) to estimate the basis.
                            *Basisptr1 = (pow_int(xpos, PowerOrder[x_bias_index_shift]) *
                                          pow_int(ypos, PowerOrder[y_bias_index_shift]) *
                                          pow_int(zpos, PowerOrder[z_bias_index_shift]));
                        }
            ...
            for (int iy = 0; iy < maxiy; iy += reduxfactor) {
                for (int ix = 0; ix < maxix; ix += reduxfactor) {
                    ...
                    if (currshortindex >= 0) {
                        ...
                        Ysum = 0;
                        for (int j = 0; j < this->numberOfClasses; j++) {
                            segPrecisionTYPE tmpexpec = (segPrecisionTYPE) Expec[currshortindex +
                                                                                 this->numElementsMasked * j];
                            Wij = tmpexpec * (invV[j]);
                            Wi += Wij;
                            Yest += Wij * (currM[j]);
                            Ysum += Wij;
                ...
                for (int iy2 = 0; iy2 < maxiy; iy2 += reduxfactor) {
                    for (int ix2 = 0; ix2 < maxix; ix2 += reduxfactor) {
                        ...
                        if (this->L2S[linearindexes2] >= 0) {
                            *WRptr += pow_int((((segPrecisionTYPE) ix2 - not_point_five_times_dims_x) *
                                               inv_not_point_five_times_dims_x), PowerOrder[0 + bfindex * 3]) *
                                      pow_int((((segPrecisionTYPE) iy2 - not_point_five_times_dims_y) *
                                               inv_not_point_five_times_dims_y), PowerOrder[1 + bfindex * 3]) *
                                      pow_int((((segPrecisionTYPE) iz2 - not_point_five_times_dims_z) *
                                               inv_not_point_five_times_dims_z), PowerOrder[2 + bfindex * 3]) *
                                      W[tempvarindex2];
                    ...
                    if (currshortindex >= 0) {
                        ...
                        for (int order = 0; order <= this->biasFieldOrder; order++) {
                            for (int xorder = 0; xorder <= order; xorder++) {
                                for (int yorder = 0; yorder <= (order - xorder); yorder++) {
                                    int zorder = order - yorder - xorder;
                                    tmpbiasfield -= FinalCoefs[ind2] * currxpower[xorder] * currypower[yorder] *
                                                    currzpower[zorder];
                                    ind2++;
                                }
                            }
...
void seg_EM_old::RunBiasField2D() {
    ...
    for (long multispec = 0; multispec < this->nu; multispec++) {
        ...
        segPrecisionTYPE AWA[((maxAllowedBCPowerOrder + 1) * (maxAllowedBCPowerOrder + 2)) / 2 *
                             ((maxAllowedBCPowerOrder + 1) * (maxAllowedBCPowerOrder + 2) / 2)] = {0.0f};
        ...
        for (int iy = 0; iy < maxiy; iy += reduxfactor) {
            for (int ix = 0; ix < maxix; ix += reduxfactor) {
                ...
                if (currshortindex >= 0) {
                    ...
                    y_bias_index_shift = 1;
                    for (int j2 = 0;
                         j2 < UsedBasisFunctions; j2++, x_bias_index_shift += 2, y_bias_index_shift += 2, Basisptr1++) {
        ...
        for (int i2 = 0; i2 < UsedBasisFunctions; i2++) {
            ...
            for (int iy = 0; iy < maxiy; iy += reduxfactor) {
                ...
                for (int ix = 0; ix < maxix; ix += reduxfactor) {
                    ...
                    currshortindex = this->L2S[linearindexes];
                    if (currshortindex >= 0) {
                        WR[i2] += pow_int((((segPrecisionTYPE) ix - not_point_five_times_dims_x) *
                                           inv_not_point_five_times_dims_x), PowerOrder[0 + i2 * 2]) *
                                  pow_int((((segPrecisionTYPE) iy - not_point_five_times_dims_y) *
                                           inv_not_point_five_times_dims_y), PowerOrder[1 + i2 * 2]) *
